...
hist1=hist(Z, freq=TRUE, breaks=seq(-4,8,l=13))$counts + 1 #+1 to adjust zero issue
hist1
hist2=hist(normal, freq=TRUE, breaks=seq(-4,8,l=13))$counts + 1 #adjust
hist2
```
...
(1)For the empirical distribution: The mean of is very close to 0. The standars deviation is close to 1. The skewness is close to 0. The kurtosis is very close to 3. Therefore, we can conclude that the characteristics of empirical and theoretical distributions are similar.

(2)From the QQ plot we can see that these points follow the line very well. The pattern is approximately linear.

The p-value is 0.03094, which is smaller than 0.05. Thus, we can say that the empirical distribution is associated with the theoretical distribution.
```{r}
#(1)
mean(Z)
sd(Z)
...
Overall, from the QQ plot and similar characteristics in Part b, we can see that this method is a good method for generating random normal distributions since its distribution is similar to the random normal distribution.   
...
The estimated variance1 is 4.414891, standard error is 1.618782. The estimation does not miss the true value because sqrt(12) is about 3.464, which is within the 1 standard error away from 4.414891. 
```{r}
library(boot)
set.seed(99)
B=1000
...
The estimated v=var1/var2 is 0.3719151, standard error is 0.4221674. The estimation does not miss the true value for the sample we generated, but it missed the true value for the population.
```{r}
set.seed(99)
...
#this doesn't use w/ repl because i define dthe population as theoretical dist
#  sample1 = rnorm(n=n1,sd=sqrt(12))
#  sample2 = rnorm(n=n2,sd=sqrt(10))
#you can use df and pull from 1st and 2nd columns w/ repl
...
#B=1000 bootstrap, collecting the ratio stat
B=1000
...
The confidence interval is ( 0.0560,  1.5161 )    
```{r}
