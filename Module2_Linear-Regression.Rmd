...
Model_Full.fit = lm(Model_Full, data=AutoNum)
summary(Model_Full.fit)
summary(Model_Full.fit)$coefficients
cat("\nThe predictors’ coefficient estimatesare shown above.")
cat("\nThe adjusted R-squared is",0.8063)
res <- resid(Model_Full.fit)
SSE=sum(res^2)
MSE= SSE/385
cat("\nThe MSE is",MSE)
```
...
cat("\nThe predictors’ coefficient estimates of all models are shown above.")
which.max(reg.summary$adjr2)
cat("\nThe best model is the model with two predictors weight and year because it has the smallest MSE and highest adjusted R-squared.\n")

summary(lm(mpg~weight))#using the summary to see df in each model
summary(lm(mpg~weight+year))
summary(lm(mpg~weight+acceleration+year))
summary(lm(mpg~displacement+weight+acceleration+year))
summary(lm(mpg~cylinders+displacement+weight+acceleration+year))
...
MSE_m5=4543.356/386
MSE_m6=4543.347/385
cat('\nadjR2_m1 =',reg.summary$adjr2[1])
cat('\nadjR2_m2 =',reg.summary$adjr2[2])
cat('\nadjR2_m3 =',reg.summary$adjr2[3])
cat('\nadjR2_m4 =',reg.summary$adjr2[4])
cat('\nadjR2_m5 =',reg.summary$adjr2[5])
cat('\nadjR2_m6 =',reg.summary$adjr2[6])
...
cat('\nMSE_m6 =',MSE_m6)
```
The best model is the model with two predictors weight and year because it has the the lowest MSE 11.74538 and highest adjusted R-squared 0.8071393. The predictors’ coefficient estimates, Radj, and MSE have been shown above.


***
Part c:

The criterion I employed to find the best subset is the lowest MSE and the largest adjusted R-squared. 
Other criteria also exist, including SSE, $MSE_{test}$, $C_p$, AIC and BIC.
...
cat("\nThe predictors’ coefficient estimates of all models are shown above.")
which.max(reg.summary$adjr2)
cat("\nThe best model is the model with two predictors weight and year because it has the smallest MSE and the highest adjusted R-squared.\n")
...
cat('\nMSE_m6 =',MSE_m6)
```
The best model is still the model with two predictors weight and year because it has the lowest MSE 11.74538 and highest adjusted R-squared 0.8071393. The predictors’ coefficient estimates, Radj, and MSE have been shown above.

***
Part e:

The results obtained from forward(b) or backward(d) is a better model with two predictors weight and year which has lower MSE and higher adjusted R-squared than that we obtained from the OLS(a). Lower MSE means that the narrower errors and higher Radj means more data points can be interpreted by the relationship in the model.
...
mse.storage
mse.storage.average=apply(mse.storage, 2, mean) # averaged mse's: train and test
mse.storage.average
cat('\nThe MSE{train} is', mse.storage.average[1])
cat('\nThe MSE{test} is', mse.storage.average[2])
```
...
```
The best model obtained from default setting is the second model which uses weight and year as predictors is the best because it has the smallest MSE_test.

***
Part c:

The MSE_test of the model of part a(OLD) is larger than the model we obtained in part b(forward), as with more predictors, there are more error terms added to the model.
...
The models in part b and part d are the same. Because the models from part b and d use the same predictors (weight+year). The coefficient estimates and predictors are the same. So, forward and backward stepwise selections finally get the same result.

***
Part f:

The models from part b and d is better than that from part a, since they have smaller MSE_test, thus less errors are contained in the model.
...
The coefficient estimates after lasso regression change so much. Estimators for displacement and acceleration change to zero. MSE is the largest in part a.
...
## Write comments, questions: Too much homwork. Too hard that it's hard for me to completely understand all parts with the help of lab resources.
