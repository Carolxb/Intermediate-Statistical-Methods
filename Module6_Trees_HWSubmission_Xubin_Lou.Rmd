...
We can see from the histogram that the people who choose not to purchased a caravan insurance policy is much more than the people who choose to purchased a caravan insurance policy. I think a small
number of predictors do not sufficient to get the good results because we have only 86 variable, which is a small number.
...
m1
cat("\nThe accuracy for train set with classification tree is", m1)
...
cat("\nThe accuracy for train set with bagging approach is", x1)
...
#Random tree
library(randomForest)
strg =matrix(rep(0,2*6),ncol=2,nrow=6)
x<-c(1:6)
n=1
for (val in x)
{
...
strg1 =matrix(rep(0,2*6),ncol=2,nrow=6)
x<-c(1:6)
n=1
for (val in x)
{
...
strg2 =matrix(rep(0,2*6),ncol=2,nrow=6)
x<-c(1:6)
n=1
for (val in x)
{
...
a1=sum(yhat.bag==Caravan.train[,86])/length(yhat.bag)
a1
cat("\nThe accuracy for train set with random forests is", a1)
...
cat("\nThe accuracy for train set with boosting is", boost.train.accuracy)
...
Comparing the five models (parts b-f) in Question#1, we find that in the classification trees and random forests, there are the same highest accuracy for the test set. So, classification trees and random forests are the best in terms of accuracy.
...
cat("\nThis score is called precision.")
ct1_te
...
In the second model of classification tree and the fourth model of random forest, the precision are NaN %, which means 0/0. because there is nobody who is predicted to make a purchase.

As we can see, the precision scores for these models are unusually small and even NaN according to the models we get from finding the larger test accuracy. Because of the imbalanced data issue and sparsity, this means that using Accuracy (TP+TN)/ALL which need to include all data into consideration is improper. Thus, the precision TP/(TP+FP) which focus on the true part data is better than the accuracy to evaluate models. Thus, the accuracy score is not an appropriate approach to evaluate the performances of the models. Therefore, I prefer to use precision scores. 
...
summary(boost.caravan)
```
As we can see in the graph, the variables PBRAND, PPERSAYT, MOSTYPE, and APERSAUT are the most important in the random forest model.
...
I don't think it's a good model since as I run the code above, I find the accuracy when I choose all No for the dataset is about 94.02267%. This means we can at least get such accuracy of 94% even when we directly guess the result as all No in the dataset. Therefore, I think this model with the accuracy of 94% is not good.


***
Part d - BONUS:

We can use under sample majority class to improve the model and relieve the problem from the imbalanced dataset. As we can see there are larger number of "No" than the number of "Yes" in the dataset, so we can change the 4000 size train model to a smaller one and remove some size of "No"'s observation in it to get a smaller but more balanced train set. In this way, we can solve the imbalanced data problem and improve the model.

***
Part e - BONUS:

After scaling the data and make a new tree model, we found that scores including accuracy and precision are almost the same as before.
